...
def create_model_func3(input_shape, optimizer='Adagrad', learn_rate=None, decay=0.0, momentum=0.0, activation='relu', dropout_rate=0.5):
    logging.debug('input_shape {}'.format(input_shape))
    logging.debug('input_shape {}'.format(type(input_shape)))
    ...
    # Hardcoding
    #input_shape_top_model = (7, 7, 512)                                                                     # VGG bottleneck layer - block5_pool (MaxPooling2D)
    ...
    # input_shape = (7, 7, 512)                                                        # 224,224,3
    # inputs = Input(shape=(input_shape))
    # model_top = Model(inputs=inputs , outputs=[predictions_class, predictions_iou])
    # model_top.load_weights(top_model_weights_path)
    # model.load_weights('my_model_weights.h5', by_name=True)

    # model_top.load_weights(top_model_weights_path)
    # model_top.load_weights('output/best-weights-003-0.9257.hdf5')
...
def train_top_model(is_grid_search):
	...
	# Create model
        ...
        callbacks_list = [csv_log, early_stopping, tensorboard]
        logging.debug('callbacks_list {}'.format(callbacks_list))
...
def prediction_VGG16():
    ...
    dataset_train_path='dataset_btl/train'
    ...
    model_top = create_model_func3((input_shape), optimizer, learn_rate, decay, momentum, activation, dropout_rate)
    ...
    images_path_name = sorted(glob.glob('dataset_btl/train/Mixed/*.jpg'))
    ...
    for image in images_path_name:
        ...
        #img = np.array([np.array(Image.open(fname)) for fname in images_list])
    ...
    for index,preds in enumerate(prediction):
        for index2, pred in enumerate(preds):
            ...
            print('images_name_list index2 : {}     '.format(images_name_list[index2]), end='')
            for p in pred:
                #logging.debug('pred {:6f}'.format(float(p)))
                print('{:6f} '.format(float(p)), end='')
